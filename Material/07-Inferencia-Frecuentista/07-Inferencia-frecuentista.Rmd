...
en la notación de la primera igualdad enfatizamos que la estimación $plug-in$ de un parámetro de la distribución $Q$ se puede calcular a partir de la muestra $\textbf{x}$; esto es, depende de la distribución empírica $P_n$ de $P$.  
...
2. Posteriormente para estimar el error estándar $se_P(\bar{x})$. 
...
Los datos _marks_ (Mardia, Kent y Bibby, 1979) contienen una muestra que consiste en los puntajes de 88 
...
```
La muestra $\textbf{x}$ es una base de datos de $88 \times 5$, y donde $i$-ésima fila corresponde a al conjunto de medidas $\textbf{x_i}^*$ de la unidad $u_i$.
...
**Ejemplo:** supongamos que queremos estimar la kurtosis de una base de datos que consta de
799 tiempos de espera entre pulsasiones de un nervio (Cox, Lewis 1976). El estimador *plug-in* de la kurtosis es:
...
kurtosis <- rerun(B, kurtosis_boot(nerve_long$val, length(nerve_long$val))) %>% 
  flatten_dbl()
# El estimador bootstrap del error estándar de la kurtosis es
sd(kurtosis)
...
$$(\theta_{\alpha},\theta_{1-\alpha})=(\hat{\theta}- z^{(1-\alpha)}\cdot \hat{se}(\hat{\theta}), \hat{\theta}-z^{(\alpha)}\cdot \hat{se}(\hat{\theta}))$$
donde $z^{(1-\alpha)=-z^{(\alpha)$ es el cuantil de orden $1-\alpha$ de la distribución Normal estándar. 
...
La distribución bootstrap es la distribución empírica de $Q$. Por lo tanto, podemos aproximar $G$ por la distribución acumulada bootstrap $G^{-1}(\alpha) \approx \hat{\theta}^*(\alpha)$, esto es, el percentil 
...
$$(\theta_{\alpha},\theta_{1-\alpha})\approx(\hat{\theta}^*(\alpha),\hat{\theta}^*(1-\alpha))$$
...
<div style="caja">
Aproximación bootstrap del intervalo percentil:
...
**Ejemplo:** En nuestro ejemplo de la kurtosis, los cuantiles muestrales son:
...
**Ejercicio:** Comapara el intervalo anterior con los intervalos
